...
coordinates(HaulCentEnviroData) = ~ x.lon + y.lat
proj4string(HaulCentEnviroData) = proj4string(mycenter_INPFC_sp)
HaulCentEnviroData = spTransform(HaulCentEnviroData, CRS("+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs "))
HaulCentEnviroData = as.data.frame(HaulCentEnviroData)
HaulCentEnviroData = HaulCentEnviroData[,c(1:4,6,22:25,27:28)]
HaulCentEnviroData = unique(HaulCentEnviroData)

write.csv(HaulCentEnviroData, "Data/GridCellPredictions.csv")
  # GridCellPredictions = read.csv("Data/GridCellPredictions.csv")
###############################################################
...
PHpa_predictGAM = predict.gam(PH.pa.gam_best, newdata=HaulCentEnviroData, type="response", se.fit=TRUE)
PHpa_predGAM = cbind(HaulCentEnviroData, PHpa_predictGAM)
...
PHcpue_predGAM = cbind(HaulCentEnviroData, PHcpue_predictGAM)
...
colnames(ATFpa_pred_CIgam) = c("x.UTM", "y.UTM", "id2", "EEZgrid", "YEAR", "START_LATITUDE", "START_LONGITUDE", "GEAR_DEPTH", "GEAR_TEMPERATURE", "StatArea", "Haul_Join", "ATFpa_fit", "ATFpa_se.fit", "ATFpa_lowerCI", "ATFpa_upperCI")

### CPUE, ARROWTOOTH FLOUNDER ###
ATFcpue_predictGAM = predict.gam(ATF.cpue.gam_best, newdata=HaulCentEnviroData, type="response", se.fit=TRUE)
ATFcpue_predGAM = cbind(HaulCentEnviroData, ATFcpue_predictGAM)
